            ...
            <div className="space-y-2">
                {data.map((item: any, i: number) => (
                    <div key={i} className="flex justify-between items-center text-xs">
                        ...
                        <span className={`font-bold ${item.label === 'Present' ? 'text-gray-800' : item.label === 'Absent' ? 'text-gray-800' : 'text-gray-400'}`}>{item.val}</span>
                    </div>
                ))}
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 p-2">
            ...
            <div>
                <div className="flex items-center justify-between mb-2">
                    <h3 className="text-sm font-bold text-gray-700">Attendance Calender NOV - 2025</h3>
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col lg:flex-row gap-4 justify-between items-end lg:items-center">
                    <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 w-full lg:w-auto flex-1">
                        {/* Date Picker Placeholder */}
                        <div className="relative">
                            <input type="text" value="11/2025" readOnly className="w-full bg-gray-50 border border-gray-200 text-gray-600 text-xs rounded-lg px-3 py-2 focus:outline-none" />
                        </div>
            ...
            {/* Legend */}
            <div className="flex flex-wrap items-center justify-end gap-4 text-[10px] text-gray-600">
                ...
                <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded border border-gray-300 bg-white"></span> N/A</div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {staffData.map((staff, idx) => (
                                <tr key={staff.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-3 text-center border-r border-gray-100"><span className="text-gray-400 text-xs">▶</span></td>
                                    <td className="px-4 py-3 text-xs text-gray-600 border-r border-gray-100">{idx + 1}</td>
                                    <td className="px-4 py-3 border-r border-gray-100">
                                        {staff.img ? (
                                            <img src={staff.img} alt="emp" className="w-8 h-8 rounded-md object-cover shadow-sm" />
                                        ) : (
                                            <div className="w-8 h-8 rounded-md bg-gray-100 flex items-center justify-center text-gray-400 border border-gray-200"><User size={16} /></div>
                                        )}
                                    </td>
                                    <td className="px-4 py-3 text-xs font-medium text-gray-700 border-r border-gray-100">{staff.name}</td>
                                    <td className="px-4 py-3 text-xs text-gray-500 border-r border-gray-100">{staff.empId}</td>
                                    <td className="px-2 py-3 text-xs text-center border-r border-gray-100 text-gray-600">{staff.present}</td>
                                    <td className="px-2 py-3 text-xs text-center border-r border-gray-100 text-gray-600">{staff.absent}</td>
                                    <td className="px-2 py-3 text-xs text-center border-r border-gray-100 text-gray-600">{staff.missed}</td>
                                    <td className="px-4 py-3 text-xs text-gray-500 border-r border-gray-100 whitespace-nowrap">Nov-2025</td>
                                    {/* Calendar Grid Cells */}
                                    {[...Array(12)].map((_, i) => {
                                        const status = getStatusForDay(staff.statusPattern, i);
                                        return (
                                            <td key={i} className="px-1 py-3 text-center border-r border-gray-100">
                            ...
                            {/* Total Summary Row */}
                            <tr className="bg-gray-50 font-bold text-xs text-gray-600 border-t border-gray-200">
                                <td colSpan={9} className="px-4 py-3 text-right">Total Present</td>
                                {[...Array(12)].map((_, i) => (
                                    <td key={i} className="px-1 py-3 text-center border-l border-gray-200 text-[10px] text-gray-400">
                                        0
                                    </td>
                                ))}
                            </tr>
                            <tr className="bg-gray-50 font-bold text-xs text-gray-600">
                ...
                <div className="p-4 border-t border-gray-200 flex items-center justify-between bg-white">
                    ...
                    <div className="flex gap-1">
                        ...
                        <button className="px-2.5 py-1 bg-cyan-100 text-cyan-700 rounded text-xs font-medium shadow-sm">1</button>
